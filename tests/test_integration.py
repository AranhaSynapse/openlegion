...
def test_vnc_proxy_rejects_agent_token(tmp_path):
    """VNC proxy rejects requests with agent auth tokens."""
    from unittest.mock import MagicMock
...
def test_vnc_proxy_allows_no_token(tmp_path):
    """VNC proxy allows requests without auth tokens (dashboard via Caddy)."""
    from unittest.mock import AsyncMock, MagicMock, patch
    ...
    with patch("httpx.AsyncClient") as mock_client_cls:
        ...
        # No auth header — this is how dashboard users access via Caddy
        resp = client.get("/vnc/index.html")
